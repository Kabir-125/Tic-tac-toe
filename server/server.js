...
io.on("connection", (socket) => {
  socket.on("start", async (data) => {
    ...
    if (waitingPlayers.length >= 2) {
      while (waitingPlayers.length >= 2) {
        ...
        io.to(player1).emit("room_assigned", {
          room,
          opponent: players[player2].name,
          playingAs: "O",
          id: newGame.id,
        });
        io.to(player2).emit("room_assigned", {
          room,
  ...
  socket.on("game_over", async (data) => {
    ...
    if (rooms[data.room]) {
      ...
      if (data.winner === data.player1) player1.gamesWon++;

      player2.gamesPlayed = player2.gamesPlayed + 1;
      if (data.winner === data.player2) player2.gamesWon++;
      ...
      if (player1stat == null) {
        player1stat = await gamesPerDay.create({
          email: data.player1,
          date: today,
        });
      }
      ...
      if (player2stat == null) {
        player2stat = await gamesPerDay.create({
          email: data.player2,
          date: today,
        });
      }
